...
void Draw_MC_ABCD()
{
  ...
  // 1~8, 11~18
  for (int i = 1; i <= 8; i++) {
    fin[i] = new TFile(("RootFile/MonoPhotonAnalysis_" + year[1] + "_" + mass[i] + "_1.root").c_str());
    dir[i] = (TDirectory *)fin[i]->Get("HLT_Photon200");
    dir[i]->cd();

    fin[i + 10] = new TFile(("RootFile/MonoPhotonAnalysis_" + year[2] + "_" + mass[i] + "_1.root").c_str());
    dir[i + 10] = (TDirectory *)fin[i + 10]->Get("HLT_Photon200");
    dir[i + 10]->cd();

    fin[i + 20] = new TFile(("RootFile/MonoPhotonAnalysis_" + year[3] + "_" + mass[i] + "_1.root").c_str());
    dir[i + 20] = (TDirectory *)fin[i + 20]->Get("HLT_Photon200");
    dir[i + 20]->cd();

    fin[i + 30] = new TFile(("RootFile/MonoPhotonAnalysis_" + year[4] + "_" + mass[i] + "_1.root").c_str());
  ...
  TH1D *ABCD_plot[10];
  for (int i = 1; i < 9; i++) {
    ABCD_plot[i] = (TH1D *)dir[i]->Get("Flow_Energy_ABCD");
    ABCD_plot[i]->Add((TH1D *)dir[10 + i]->Get("Flow_Energy_ABCD"));
  }
  ...
  for (int i = 1; i < 9; i++) {
    ...
    mgr::DrawEntryRight("2017 2018 13TeV");
    ...
    TightX->Draw("same");
    canvas->SaveAs(("./Plots/MC_ABCD_plot1718_EnergyCut_" + mass[i] + ".pdf").c_str());
  ...
  TH1D *ABCD_plot16[10];
  for (int i = 1; i < 9; i++) {
    ABCD_plot16[i] = (TH1D *)dir[20+i]->Get("Flow_Energy_ABCD");
    ABCD_plot16[i]->Add((TH1D *)dir[30 + i]->Get("Flow_Energy_ABCD"));
  }
  ...
  for (int i = 1; i < 9; i++) {
    ...
    TightX->SetLineWidth(3);
    TightX->Draw("same");
    canvas->SaveAs(("./Plots/MC_ABCD_plot16_EnergyCut_" + mass[i] + ".pdf").c_str());
