#include <string>
using namespace std;
void MonoDrawMETTriggerResultPlot()
{
	//create a canvas
	...
	for(int y=1;y<2;y++){
	  for(int m=0;m<4;m++){
		...
		TFile *fin = new TFile (("Monopole_PFMETTrigger_"+Year[y]+"_"+GeV[m]+"_.root").c_str());
		TDirectory *dir = (TDirectory*)fin->Get("NOTRG");
//		TDirectory *dir = (TDirectory*)fin->Get("HLT_PFMET200_HBHE_BeamHaloCleaned");
//		TDirectory *dir = (TDirectory*)fin->Get("HLT_PFMETTypeOne200_HBHE_BeamHaloCleaned");
	 
		dir->cd();
		
		gStyle->SetOptStat(0);
		const int plotNum = 10;
		string plotName[10] = { "NoCut_","N1_Quality_","N1_MET_","N1_F51_","N1_dEdxSig_","N1_HLT_"
					,"Flow_Quality_","Flow_MET_","Flow_F51_","Flow_dEdxSig_"};	
		string histogram[10] = { "","N_1_HLT_Quality_","N_1_HLT_MET_","N_1_HLT_F51_",
					"N_1_HLT_dEdxSig_","N_1_HLT_HLT_","Flow_HLT_Quality_",
					"Flow_HLT_MET_","Flow_HLT_F51_","Flow_HLT_dEdxSig_"};
	...
	//MET(e55)
        MET.push_back((TH1D*)dir->Get((histogram[i]+"E55").c_str()));
        MET[i]->SetXTitle("MET(GeV)");
        MET[i]->SetYTitle("Entries");
        MET[i]->SetLineColorAlpha(28,0.5);
        MET[i]->SetFillColor(42);
        MET[i]->SetTitle("");
        MET[i]->Draw();
        mgr::SetSinglePad(canvas);
        mgr::SetAxis(MET[i]);
        ...
        leg4->AddEntry(MET[i],Mass[m].c_str(),"f");
        leg4->Draw();
        canvas->SaveAs(("./Plots/"+Year[y]+" MC 15k "+Mass[m]+"/TRG/"+plotName[i]+"MET(e55).pdf").c_str());
